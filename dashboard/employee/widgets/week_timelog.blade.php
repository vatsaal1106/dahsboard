@if (in_array('week_timelog', $activeWidgets) &&
    ...
    <div @class([
        'mb-3',
        'col-md-6' => in_array('lead', $activeWidgets) && $leadAgent,
        'col-md-12' => !(in_array('lead', $activeWidgets) && $leadAgent),
    ])>
        <div class="bg-white p-20 border shadow-md !rounded-lg d-flex justify-content-between align-items-center">
            <div class="d-block text-capitalize w-100">
                <h5 class=" f-18 mb-20 text-darkest-grey font-bold">@lang('modules.dashboard.weekTimelog') <span
                        ...
                        <ul class="pagination pagination-sm week-pagination">
                            @foreach ($weekPeriod->toArray() as $date)
                                <li @class([
                                    ...
                                    'week-timelog-day',
                                    'active' =>
                                        now(company()->timezone)->toDateString() == $date->toDateString(),
                                ]) data-toggle="tooltip"
                                    data-original-title="{{ $date->translatedFormat(company()->date_format) }}"
                    ...
                    <div class="progress" style="height: 20px;">
                        @php
                            ...
                            $totalDayMinutesPercent =
                                $totalDayMinutes > 0
                                    ? floatval(
                                        (floatval($totalDayMinutes - $totalDayBreakMinutes) / $totalDayMinutes) * 100,
                                    )
                                    : 0;
                        @endphp
                        <div class="progress-bar bg-primary" role="progressbar"
                            style="width: {{ $totalDayMinutesPercent }}%" aria-valuenow="{{ $totalDayMinutesPercent }}"
                            aria-valuemin="0" aria-valuemax="100" data-toggle="tooltip"
                            data-original-title="{{ minute_to_hour($totalDayMinutes - $totalDayBreakMinutes) }}"></div>

                        <div class="progress-bar bg-grey" role="progressbar"
                            style="width: {{ 100 - $totalDayMinutesPercent }}%"
                            aria-valuenow="{{ $totalDayMinutesPercent }}" aria-valuemin="0" aria-valuemax="100"
                            data-toggle="tooltip" data-original-title="{{ minute_to_hour($totalDayBreakMinutes) }}">
                        </div>
                    </div>
