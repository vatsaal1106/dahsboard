    ...
    <div class="page-header-right float-right d-flex align-items-center justify-content-end">
        ...
        @if (isWorksuiteSaas())
            @if (session('impersonate'))
                <x-forms.link-primary icon="stop" data-toggle="tooltip"
                    ...
                    :link="route('superadmin.superadmin.stop_impersonate')" class="mr-3 px-1">
                    @lang('superadmin.stopImpersonation')
                </x-forms.link-primary>
            @endif
            {{-- SAAS --}}
            @if (in_array(company()->package->default, ['yes']) && in_array('admin', user_roles()))
                <a href="{{ route('billing.upgrade_plan') }}" class='btn-primary btn btn-sm rounded mr-3 f-14 py-2 px-3'
                    data-toggle="tooltip" data-original-title="{{ __('superadmin.packages.upgradePlan') }}">
                    <i class="bi bi-stars"></i>
                    @if (is_null($selfActiveTimer))
                        {{ __('superadmin.packages.upgradePlan') }}
                    @endif
                </a>
            @elseif (in_array(company()->package->default, ['trial']) && in_array('admin', user_roles()))
                @php
                    $daysLeftInTrial = now(company()->timezone)->diffInDays(
                        \Carbon\Carbon::parse(company()->licence_expire_on)->addDays(1),
                        false,
                    );
                    $buttonClass = $daysLeftInTrial >= 0 ? 'btn-warning' : 'btn-danger';
                @endphp
                <a href="{{ route('billing.upgrade_plan') }}"
                    class="{{ $buttonClass }} border btn btn-sm rounded mr-3 f-14 py-2 px-3 font-weight-semibold d-none d-lg-block"
                    data-toggle="tooltip" data-original-title="{{ __('superadmin.packages.upgradePlan') }}">

                    <i @class([
                        'bi bi-circle-fill',
                        'text-success' => $daysLeftInTrial >= 0,
                        'text-danger' => $daysLeftInTrial < 0,
                    ])></i>
                    @if (is_null($selfActiveTimer))
                        {{ $daysLeftInTrial > 0 ? $daysLeftInTrial . ' ' . __('superadmin.packages.daysLeftTrial') : __('superadmin.packages.trialExpired') }}
                    @endif
                </a>
            @endif
        ...
        <ul>
            @if (checkCompanyPackageIsValid(user()->company_id))
                <!-- SEARCH START -->
                <li data-toggle="tooltip" data-placement="top" title="{{ __('app.search') }}"
                    class="d-none d-sm-block px-1 pl-2">
                    <div class="d-flex align-items-center">
                        <a href="javascript:;" class="d-block header-icon-box open-search">
                            <i class="fa fa-search text-xl text-dark-grey"></i>
                        </a>
                    </div>
                </li>
                <!-- SEARCH END -->
                ...
                <li data-toggle="tooltip" data-placement="top" title="{{ __('app.menu.stickyNotes') }}"
                    ...
                    <div class="d-flex align-items-center">
                        <a href="{{ route('sticky-notes.index') }}" class="d-block header-icon-box openRightModal">
                            <i class="fa fa-sticky-note text-xl text-dark-grey"></i>
                        </a>
                    </div>
                </li>
                <!-- Sticky Note END -->
                ...
                @if (!in_array('client', user_roles()))
                    ...
                    @if (in_array('timelogs', user_modules()) &&
                        ...
                        <!-- START TIMER -->
                        <li class="px-1" data-toggle="tooltip" data-placement="top"
                            title="{{ __('modules.timeLogs.startTimer') }}">
                    ...
                    <!-- ADD START -->
                    <li class="px-1" data-toggle="tooltip" data-placement="top" title="{{ __('app.createNew') }}">
                        <div class="add_box dropdown">
                            <a class="d-block dropdown-toggle header-icon-box" type="link" data-toggle="dropdown"
                                aria-haspopup="true" aria-expanded="false">
                                <i class="fa fa-plus-circle text-xl text-dark-grey"></i>
                            </a>
                            <!-- DROPDOWN - INFORMATION -->
            ...
            <!-- NOTIFICATIONS END -->
            <!-- LOGOUT START -->
            <li class="px-1 pr-2" data-toggle="tooltip" data-placement="top" title="{{ __('app.logout') }}">
                <div class="logout_box">
                    <a class="d-block header-icon-box" href="javascript:;"
                        onclick="event.preventDefault();
                    document.getElementById('logout-form').submit();">
                        <i class="fa fa-power-off text-xl text-dark-grey"></i>
                    </a>
                </div>
            </li>
            <!-- LOGOUT END -->
