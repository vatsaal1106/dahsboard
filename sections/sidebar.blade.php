        ...
        <div class="sidebar-brand-box dropdown cursor-pointer {{ user()->dark_theme ? 'bg-dark' : '' }}">
                ...
                @if (companyOrGlobalSetting()->sidebar_logo_style !== 'full')
                    ...
                    <div class="sidebar-brand-name">
                        <h1 class="mb-0 f-16 f-w-500 text-white-shade mt-0" data-placement="bottom"
                            data-toggle="tooltip" data-original-title="{{ $appName }}">{{ $appName }}
            ...
            <div class="dropdown-menu dropdown-menu-right sidebar-brand-dropdown ml-3"
                ...
                <div class="d-flex justify-content-between align-items-center profile-box">
                    <a
                        @if (in_array('client', user_roles())) href="{{ route('profile-settings.index') }}" @elseif (user()->is_superadmin) href="{{ route('superadmin.settings.super-admin-profile.index') }}" @else href="{{ route('employees.show', user()->id) }}" @endif>
                        <div class="profileInfo d-flex align-items-center mr-1 flex-wrap">
                            <div class="profileImg mr-2">
                                <img class="h-100" src="{{ $user->image_url }}" alt="{{ user()->name }}">
                            </div>
                    ...
                    @if (user()->is_superadmin)
                        <a href="{{ route('superadmin.settings.super-admin-profile.index') }}" data-toggle="tooltip"
    ...
    <div
        ...
        <div class="d-flex align-items-center">
            @if (isWorksuite() || user()->is_superadmin)
                <p class="mb-0 text-dark-grey px-1 py-0 rounded f-10">
                    v{{ \Illuminate\Support\Facades\File::get('version.txt') }}</p>
            @endif
            @if (isWorksuiteSaas())
                ...
                @elseif(user()->is_superadmin && !global_setting()->frontend_disable)
                    <p class="mb-0"><a target="_blank" data-toggle="tooltip"
                            data-original-title="{{ __('superadmin.VisitFrontWebsite') }}"
                            href="{{ route('front.home') }}" class="text-secondary ml-2 f-15"><i
